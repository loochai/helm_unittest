...
func (m MyStringList) Less(i, j int) bool {
	s1, s2 := "", ""
	if v, ok := m[i].(map[interface{}]interface{}); ok {
		for key, value := range v {
			if key == "name" {
				s1 = fmt.Sprintf("%v", value)
				//fmt.Println(key)
			}
		}
	}

	if v, ok := m[j].(map[interface{}]interface{}); ok {
		for key, value := range v {
			if key == "name" {
...
func (a EqualValidator) Validate(context *ValidateContext) (bool, []string) {
	...
	for idx, manifest := range manifests {
		...
		// sort orderly when use equal
		if m, ok := a.Value.([]interface{}); ok {
			sort.Sort(MyStringList(m))

		}
		if m, ok := actual.([]interface{}); ok {
			sort.Sort(MyStringList(m))
		}
